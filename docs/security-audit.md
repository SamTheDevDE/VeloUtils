...
[Documentation](README.md) · [Security policy](../SECURITY.md) · [Architecture](architecture.md)

> This is the technical security record. Server owners should follow the simpler [security checklist](../SECURITY.md).
...
| Remote commands | Disabled by default, with authenticated transport and allowlists on both ends when enabled |
...
Protocol authentication is optional only to make first-run and standalone development possible. A combined production deployment should treat `authentication.required: false` as an unsafe state.

## Abuse resistance

- Protocol payloads and input fields have explicit maximum lengths.
- Replay and request-tracking maps have size limits and expire old entries.
...
| Optional feature appears configured but is inactive | Check [Implementation status](implementation-status.md) before deployment |
